...
После `recover` deferred-функция завершается, а функция, в которой произошла
паника, возвращает управление вызывающему коду; выполнение не продолжается с
точки после `panic` внутри той же функции.
